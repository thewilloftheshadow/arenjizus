...
export default class Ping extends SlashCommand {
    ...
    override async run(interaction: CommandInteraction) {
        ...
        if (player.discordId) await interaction.guild?.members.resolve(player.discordId)?.roles.remove("1087498947389571157").catch(() => {})
        if (player.discordId) await interaction.guild?.members.resolve(player.discordId)?.roles.add("1087498947389571158").catch(() => {})
