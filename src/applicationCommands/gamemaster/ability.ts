			...
			options: [
				{
					...
					options: [
						...
						},
						{
							...
							name: "uses",
							description:
								"The number of uses the ability has (before being recharged by GMs)",
							required: true
						}
					]
				},
						...
						},
						{
							...
							options: [
								...
								},
								{
									...
									name: "give_item",
									description:
										"The name of the item that this ability is given with",
									required: false,
									autocomplete: true
								},
								{
									...
									name: "subtract_item",
									description:
										"The name of the item to subtract when this ability is used",
									required: false,
									autocomplete: true
								}
							]
		...
		switch (subcommand) {
			case "create": {
				...
				return interaction.editReply(
					`Created ability ${name} with description ${description} and ${uses} uses`
				)
			}
			case "grant": {
				...
				if (done.isErr()) return interaction.editReply(done.unwrapErr())
				return interaction.editReply(
					`Granted ability ${abilityName} to player ${playerName}`
				)
			}
			case "revoke": {
				const playerName = interaction.options.getString("player", true)
				...
				if (done.isErr()) return interaction.editReply(done.unwrapErr())
				return interaction.editReply(
					`Revoked ability ${abilityName} from player ${playerName}`
				)
			}
			case "reset_uses": {
				...
				for (const ability of abilities) {
					await resetAllAbilityUses(ability)
				}
				if (errors.length > 0) return interaction.editReply(errors.join("\n"))
				return interaction.editReply(
					`Reset daily uses for ${abilities.length} abilities`
				)
			}
			case "view": {
				const abilityName = interaction.options.getString("ability", true)
				const hideUsers =
					interaction.options.getBoolean("hide_users", false) || false
			...
			case "item": {
				if (subcommandGroup === "link") {
					...
					const subtractItemName = interaction.options.getString(
						"subtract_item",
						false
					)
					if (giveItemName) {
						...
						if (!item)
							return interaction.editReply(`No item with name ${giveItemName}`)
					...
					if (subtractItemName) {
						...
						if (!item)
							return interaction.editReply(
								`No item with name ${subtractItemName}`
							)
						await database.abilityItemLink.upsert({
							where: {
								abilityName_itemName: {
			...
			case "set_uses": {
				...
				if (!player)
					return interaction.editReply(`No player with name ${playerName}`)
				const uses = interaction.options.getInteger("uses", true)
