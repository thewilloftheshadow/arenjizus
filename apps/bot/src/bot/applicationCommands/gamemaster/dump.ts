...
export default class Ping extends ApplicationCommand {
	...
	override async run(interaction: ChatInputCommandInteraction) {
		...
		if (!this.client.__dirname.startsWith("/home/ubuntu/arenjizus"))
			return interaction.editReply(
				"This command can only be used the production environment (yay jank code)"
			)
		const newName = `backup_${Date.now()}.db`
		copyFileSync(
			`/home/ubuntu/arenjizus/packages/database/dev.db`,
			`/home/ubuntu/arenjizus/packages/database/${newName}`
		)
