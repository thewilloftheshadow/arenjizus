...
export default class AutoCompleteHandler {
    ...
    public loadAutoCompletes() {
        this.client.functions.getFiles(`${this.client.__dirname}/dist/src/bot/autoCompletes`, "", true).forEach((parentFolder) =>
            this.client.functions.getFiles(`${this.client.__dirname}/dist/src/bot/autoCompletes/${parentFolder}`, ".js").forEach(async (fileName) => {
                const autoCompleteFile = await import(`../../src/bot/autoCompletes/${parentFolder}/${fileName}`)
                // eslint-disable-next-line new-cap
                const autoComplete: AutoComplete = new autoCompleteFile.default(this.client)
                return this.client.autoCompletes.set(autoComplete.name, autoComplete)
            }))
    }
    ...
    private fetchAutoComplete(name: string): AutoComplete | undefined {
        return this.client.autoCompletes.find((autoComplete) => autoComplete.name === name)
    }
     ...
     */
    private async runAutoComplete(autoComplete: AutoComplete, interaction: AutocompleteInteraction) {
        autoComplete
            .run(interaction)
