...
export default class ButtonHandler {
     ...
     */
    public loadButtons() {
        this.client.functions.getFiles(`${this.client.__dirname}/dist/src/bot/buttons`, "", true).forEach((parentFolder) =>
            this.client.functions.getFiles(`${this.client.__dirname}/dist/src/bot/buttons/${parentFolder}`, ".js").forEach(async (fileName) => {
    ...
    public async handleButton(interaction: ButtonInteraction) {
        ...
        if (interaction.customId.includes(":")) {
            ...
            const user = buttonId.split(":")[1]
            if (user !== interaction.user.id) return interaction.reply({ content: "This button is not for you!", ephemeral: true })
        }
    ...
    private async runButton(button: Button, interaction: ButtonInteraction) {
        ...
        button.run(interaction).catch(async (error): Promise<any> => {
        ...
        return setTimeout(() => this.coolDowns.delete(interaction.user.id), this.coolDownTime)
    }
}
