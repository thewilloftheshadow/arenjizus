...
export default class DropDown {
     ...
     */
    public validate(interaction: SelectMenuInteraction): MessageEmbedOptions | null {
        ...
        if (this.ownerOnly && interaction.guild?.ownerId !== interaction.user.id) {
            return {
                title: "Missing Permissions",
                description: "This drop down can only be ran by the owner of this guild!",
            }
        }
        ...
        if (interaction.guild && this.permissions.length && !interaction.memberPermissions?.has(this.permissions)) {
            return {
                title: "Missing Permissions",
                description: `You need the ${this.permissions.join(", ")} permission${this.permissions.length > 1 ? "s" : ""} to run this drop down.`,
            }
        }
    ...
    public async preCheck(_interaction: SelectMenuInteraction): Promise<[boolean, MessageEmbedOptions?]> {
        return [true]
    }
