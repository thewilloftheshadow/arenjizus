...
export default class SlashCommandHandler {
     ...
     * @returns The slash command we've found.
     */
    ...
    public async handleCommand(interaction: CommandInteraction) {
        ...
        if (!command) {
            ...
            const sentryId = await this.client.logger.sentry.captureWithInteraction(new Error(`Non existent slash command invoked`), interaction)
            if (process.env.NODE_ENV === "production") this.client.application?.commands.delete(interaction.commandName)
            else {
                await Promise.all(
                    this.client.guilds.cache.map((guild) =>
                        guild.commands.delete(interaction.commandName).catch((error) => {
                            if (error.code === 50001) {
                                this.client.logger.error(
                                    null,
                                    `I encountered DiscordAPIError: Missing Access in ${guild.name} [${guild.id}] when trying to set slash commands!`
                                ...
                                this.client.logger.sentry.captureWithExtras(error, {
                                    ...
                                    "Guild ID": guild.id,
                                    "Slash Command Count": this.client.slashCommands.size,
                                    "Slash Commands": this.client.slashCommands.map((cmd) => ({
                                        name: cmd.name,
    ...
    private async runCommand(command: SlashCommand, interaction: CommandInteraction): Promise<any> {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        command.run(interaction).catch(async (error): Promise<any> => {
            this.client.logger.error(error)
