...
export default class DropdownHandler {
    ...
    public async handleDropDown(interaction: SelectMenuInteraction) {
        ...
        if (!dropDown || (process.env.NODE_ENV === "development" && !this.client.functions.isAdmin(interaction.user.id))) return
    ...
    private async runDropDown(dropdown: DropDown, interaction: SelectMenuInteraction): Promise<any> {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        dropdown.run(interaction).catch(async (error): Promise<any> => {
